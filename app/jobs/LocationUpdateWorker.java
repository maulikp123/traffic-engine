		...
		try {
			...
			synchronized(Application.graph.getVehicleUpdateLock(vehicleId)) {
				...
				for(Location location : locationUpdate.getLocationList()) {
					...
					// added by maulik for test
					StreetEdge edge = Application.graph.getEdgeFromCoord(vo.getPosition());
					System.out.println("Closest edge is " + edge);
					Integer id = Application.graph.getEdgeId(edge);
					System.out.println("Edge Id is " + id);
					Double speed = TrafficStats.getEdgeSpeed(id, observationTime);
					System.out.println("Speed on this edge is " + speed);
